...
class FeatureExtractor(object):
    ...
    def extract_features(tokens, buffer, stack, arcs):
        ...
        if stack:
            ...
            try:
                stack_idx1 = stack[0]
                token_idx1 = tokens[stack_idx1]

                if 'tag' in token_idx1 and FeatureExtractor._check_informative(token_idx1['tag']):
                    tags = token_idx1['tag'].split("|")
                    for tag in tags:
                        result.append('STK_1_TAG_' + tag)
        ...
        if buffer:
            ...
            try:
                ...
                token_idx1 = tokens[buffer_idx1]
                if FeatureExtractor._check_informative(token_idx1['word'], True):
                    result.append('BUF_1_FORM_' + token_idx1['word'])    
                
                if 'tag' in token_idx1 and FeatureExtractor._check_informative(token_idx1['tag']):
                    tags = token_idx1['tag'].split("|")
                    for tag_new in tags:
                        result.append('BUF_1_TAG_' + tag_new)
            ...
            """try:
                ...
                token_idx2 = tokens[buffer_idx2]
                #if FeatureExtractor._check_informative(token_idx1['word'], True):
                #    result.append('BUF_1_FORM_' + token_idx1['word'])    
                
                if 'ctag' in token_idx2 and FeatureExtractor._check_informative(token_idx1['ctag']):
                    tags = token_idx2['ctag'].split("|")
                    for tag_new in tags:
                        result.append('BUF_2_CTAG_' + tag_new)
                
                dep_left_most, dep_right_most = FeatureExtractor.find_left_right_dependencies(buffer_idx1, arcs)
                if FeatureExtractor._check_informative(dep_left_most):
                    result.append('BUF_2_LDEP_' + dep_left_most)
                if FeatureExtractor._check_informative(dep_right_most):
                    result.append('BUF_2_RDEP_' + dep_right_most)
            except IndexError:
                #return result        
                pass"""
